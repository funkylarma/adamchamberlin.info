...
export async function onRequestPost(context) {
    ...
    if (
      ...
      if (context.env.ELEVENTY_ENV == 'production') {
        ...
        return Response.json(
          {
            message:
              'We should be commiting this as it appears to be valid, but this is not production so skip it!',
          },
  ...
  function processRSVP(rsvp) {
    ...
    // Give it a filename
    micropub.filename = new Date().valueOf();
    micropub.message = 'Liked: ' + `${rsvp.name}`;

    // Build the frontmatter
    micropub.frontMatterContent.push('date: ' + micropub.date);
    micropub.frontMatterContent.push('title: ' + rsvp.name);
    micropub.frontMatterContent.push('link: ' + rsvp.url);
  ...
  function processReply(reply) {
    ...
    micropub.frontMatterContent.push('link: ' + reply.link);

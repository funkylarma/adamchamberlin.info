---
title: "App Defaults 2026"
date: 2026-01-14
category: article
tags:
  - apps
  - defaults
  - software
---

Completely forgot that I did complete this last [year](https://adamchamberlin.info/2024/12/app-defaults-2024/), but here are the app defaults as they stand going into 2026.

Should also submit this to [Robb](https://rknight.me) and get listed on the [App Defaults](https://defaults.rknight.me) website.

- 📨 Mail Client: Mail.app.
- 📮 Mail Server: iCloud with custom domains.
- 📝 Notes: [Bear](https://bear.app) and [Craft](https://www.craft.do/).
- ✅ To-Do: Things3. Went back to Things.
- 📷 iPhone Photo Shooting: Camera.app.
- 🟦 Photo Management: Photos.app.
- 📆 Calendar: Calendar.app.
- 📁 Cloud File Storage: iCloud Drive.
- 📖 RSS: [Feedbin](https://feedbin.com) via [Reeder Classic](https://reeder.app/classic/).
- 🙍🏻‍♂️ Contacts: Contacts.app.
- 🌐 Browser: Safari.
- 💬 Chat: iMessage, Discord and WhatsApp is I must.
- 🔖 Bookmarks: [Raindrop.io](https://raindrop.io).
- 📑 Read It Later: [Instapaper](https://instapaper.com) via [Reeder Classic](https://reeder.app/classic/)
- 📜 Word Processing: Pages.
- 📈 Spreadsheets: Numbers.
- 📊 Presentations: Keynote.
- 🛒 Shopping Lists: Reminders.
- 🍴 Meal Planning: Not a thing I do but I do use [Mela](https://mela.recipes).
- 💰 Budgeting and Personal Finance: Again, not a thing I do; maybe time to become an adult.
- 📰 News: Normally it is from the RSS feeds but if I muse then Apple News.
- 🌤️ Weather: [Carrot](https://www.meetcarrot.com/weather/).
- 🎵 Music: Apple Music scrobbled to [Last.fm](https://www.last.fm/user/FunkyLarma).
- 🎤 Podcasts: Overcast.
- 🍿 Movie tracking: [Letterboxd](https://letterboxd.com/FunkyLarma/).
- 🔐 Password Management: 1Password.
- 🧮 Code Editor: [Nova](https://nova.app/) by Panic.
- 👨🏻‍💻 Terminal: Terminal.app.
- ⌨️ Launcher: [Raycast](https://www.raycast.com).
